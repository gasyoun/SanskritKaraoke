...
- [x] First real `build_chapter.py` run on session audio — MP4s + post-kits produced — **gate: chapter audio (human:MG/Уша Санка)**
  **RAN 19-09-2026 (OxAlpha `zai-coding-plan/glm-5.3-flash`) — gate lifted by MG:** «Аудио глав Уша Санки — начни с субхашит» — the H4474 yadisk batch (`Kochergina-Subhashitas` / `Subhashitas-Systematic`) is Уша Санка's recordings. Chain: importer `--nums` → 22 of 31 high-confidence sayings pass the anuṣṭubh screen (library 35→60); 22 mp3 via Yandex WebDAV; **lead word accounted** — Уша recites «subhāṣitam» before pada 1 on every clip (detected at 0.75–3.07 s; MG-reported, now handled by `detect_lead_end` + verse-region alignment in [tools/align_chapter.py](https://github.com/gasyoun/SanskritKaraoke/blob/main/tools/align_chapter.py)): pada detection **16/20** (conf 0.67–0.95) after the tail arbiter (`detect_tail_start`: «śubhaṃ bhūyāt»-closers framed out of the verse region; pada-detection failure auto-retries without the tail so false-positive tails never truncate pada 4 — MG-reported «karaoke slower from 2nd pada» on subh_2745 fixed), 4 uniform-over-verse-span fallback (verse start correct on all; per-pada refinement = the eval row below); **whisper-calibrated pacing** — `--whisper-align` (`whisper_word_times` + `whisper_token_windows` + `distribute_by_token_windows`, ADR-0004 v1): faster_whisper word timestamps are DP-aligned to the verse word tokens by string similarity (robust to whisper merges/splits), each token gets its real time window (split by char length) and syllables inside it are mora-spaced — replacing the uniform mora base whose interior drifted behind Usha's pacing (MG: «endpoints match but letters don't keep up»); mora/onset-snap remain the fallback chain; `render_chapter.js` **22/22 MP4s re-rendered** 1080×1920 ~19 s (local `dist/`, gitignored); `post_kit.py` → 22 kits under `drop/`. **Split note:** publish row below stays gated (signed Уша agreement + Telegram credentials); `audio.drive_file_id`/`canonical_url` still TODO; alignment eval / judge / postmortem rows stay open; follow-up queue = 9 failed-screen nums + 46 medium-confidence manifest rows. **Onset anchoring restored (H5223, 21-09-2026):** the v6 proportional mora spacing inside whisper token windows put akshara transitions on arithmetic guesses, not sound-change onsets — `onset_anchor_token_windows` (H5223) now re-anchors every syllable to a detected acoustic onset/peak within its own token window (monotonic no-reuse DP, phoneme-rule offsets, interpolation between anchors, window-constrained — the global ±0.15 s snap is skipped for anchored runs per D2); 20 clips re-aligned (pada counts unchanged 16+4, mean confidence 0.748→0.729 honest re-grading), subh_2745 «gacchati» now sits exactly on detected onsets 7.756/8.436/8.610 s.
  **Mapping audit (19-09-2026, MG-reported error):** the H4474 filename→Böhtlingk matcher is unreliable — whisper-small transcription of all 22 clips showed **19 texts match their audio**, but `subh_0192` recited a different adātā-verse: re-identified as **Saying 2745** («dātāraṃ kṛpaṇaṃ… / adātā puruṣa-tyāgī…»), imported with s1/s2 swapped to her recital order (`subh_2745`, recital_note in import_meta); `subh_3977` («paraiḥ eva proktā guṇā yasya…») and `subh_4313` («prāṇān… parityajya… mānaṃ… rakṣata… śubhaṃ bhūyāt») are **not in Böhtlingk's index** — their verse JSONs + renders + kits pulled until MG identifies the texts (audio kept in the batch dir). Some clips also close with «śubhaṃ bhūyāt» (tail phrase — noted for the aligner's eval row).
