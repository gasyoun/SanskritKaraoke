...
- **Bare `/drain` truth-pass — ROADMAP.md Phase 2 stale checkboxes verified and ticked** (OxAlpha `z-ai/glm-5.3-flash`, 28-08-2026). Auto-split Bucket-A pick landed on the TTS checkbox, which the 18-08-2026 truth-pass banner bars (deliberately not scheduled, K5 product-premise) — left open. The remaining three Phase 2 checkboxes were verified shipped on `main` and ticked with evidence: forced-alignment pipeline ([tools/align_chapter.py](https://github.com/gasyoun/SanskritKaraoke/blob/main/tools/align_chapter.py) ADR-0003 CLI + [tools/eval_alignment.py](https://github.com/gasyoun/SanskritKaraoke/blob/main/tools/eval_alignment.py) gold harness, superseded by [ADR-0004](https://github.com/gasyoun/SanskritKaraoke/blob/main/docs/adr/0004-approved-timing-corpus-alignment.md); Whisper = Phase B cross-check only); UI review mode (`teAutoTimingAndSwitch` app.js:4336, orange <0.5-confidence flags, Tab/Shift-Tab `_teJumpToNextUncertain` app.js:5161); verse agent (repo half: [tools/build_chapter.py](https://github.com/gasyoun/SanskritKaraoke/blob/main/tools/build_chapter.py) align→render→post-kit chain; live run gated on chapter audio — Lane B1/G2). Docs-only pass, no code changed.

### Added

- **H3570 — spine wiring: FINDINGS pointer + "How this repo is wired" README section** (Opus 5 `claude-opus-5`, 27-08-2026, [#106](https://github.com/gasyoun/SanskritKaraoke/pull/106)). This repo's slice of the fourteen-repo spine-interconnection programme, per [docs/PLAN_SANSKRITKARAOKE_INTERCONNECTION_2026-08.md](https://github.com/gasyoun/SanskritKaraoke/blob/main/docs/PLAN_SANSKRITKARAOKE_INTERCONNECTION_2026-08.md). `CLAUDE.md` gains the ruling-F1 pointer naming which hub FINDINGS to append to — infra and process to [Uprava](https://github.com/gasyoun/Uprava/blob/main/FINDINGS.md), Sanskrit data to [SanskritLexicography](https://github.com/gasyoun/SanskritLexicography/blob/master/FINDINGS.md) — and states that this repo keeps no registries of its own, so a future session does not read the absence as neglect. `README.md` gains the default-F11 orientation section written against verification Gate 4: what this repo produces that others consume (the H3261 caption exporter, now [shared-code family 28](https://github.com/gasyoun/github-spine/blob/main/SHARED_CODE.md); the verse records; `rv_verse_seeds.json` as seed/lookup only), what it consumes (RU glosses from the 1.09 M-row Sa→Ru corpus alignment lexicon — query it, never re-align here), where to record a gotcha, and one-click links to PROJECT_INTERLINKS, SHARED_CODE, FEATURES_INDEX and the GTD hub. Baseline before this pass: zero SHARED_CODE rows, zero README spine back-links, wiring score 46/100.
