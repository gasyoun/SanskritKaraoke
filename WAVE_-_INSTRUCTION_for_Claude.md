...
## Рабочие файлы
- Последняя версия: **v1.265** → `/mnt/user-data/outputs/html_v265.zip`
- Рабочая копия: `/home/claude/project/`
- История версий: `/home/claude/project/ver_info.txt`
...
### Файлы
- `index.html` — весь интерфейс + встроенная справка
- `src/scripts/app.js` — весь JS (~480KB, бинарный UTF-8, читать через python3)
- `src/style.css` — стили
- `apte_prosody.html` / `apte_prosody_ru.html` — справочник метров
...
- Тэппинг временно отключён (кнопка скрыта)
- При замене файла на Drive старый может не удалиться
